...
exports.createFirstAdmin = functions.https.onRequest((req, res) => {
  const auth = admin.auth();
  auth
    .setCustomUserClaims("vMF6R8yWCLObVtbxRZiLiSXYjHq2", { role: "admin" })
    .then(() => {
      auth
        .getUserByEmail("stsai123@terpmail.umd.edu")
        .then((userRecord) => {
          const role = userRecord.customClaims["role"];
          res.json({ result: `stsai123@terpmail.umd.edu role is ${role}` });
        })
        .catch((error) => {
