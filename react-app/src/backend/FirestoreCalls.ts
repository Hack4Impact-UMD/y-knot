...
import { Attendance, Course, CourseID, Homework } from '../types/CourseType';
import {
  LeadershipApplicant,
  Student,
  StudentAttendance,
...
// Merges student A and B into mergedStudent
export function mergeStudents(
  ...
  return new Promise((resolve, reject) => {
    runTransaction(db, async (transaction) => {
      ...
      var courseList = mergedStudent.courseInformation;
      var coursePromises = [];
      ...
      for (let i = 0; i < courseRefList.length; i++) {
        ...
        // Remove student A and B from course
        course.students = course.students.filter(function (s) {
          return s !== studentAId && s !== studentBId;
        });

        // Add merged student to course
        course.students.push(newStudentRef.id);
        promisesList.push(
          transaction.update(doc(db, 'Courses', courseId), {
            students: course.students,
          }),
      ...
      promisesList.push(transaction.set(newStudentRef, mergedStudent));

      // Delete student A and student B
      promisesList.push(transaction.delete(doc(db, 'Students', studentAId)));
      promisesList.push(transaction.delete(doc(db, 'Students', studentBId)));
      await Promise.all(promisesList);
    })
      .then(() => {
        resolve();
      ...
      .then((snapshot) => {
        snapshot.docs.map((doc) => {
          ...
          if (studentMatch.matches.includes(studentTwoId)) {
            ...
            // If studentMatch.matches is empty, delete the document.
